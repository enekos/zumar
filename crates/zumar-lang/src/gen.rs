...
/// Where the emitted crate is headed.
///
/// - `Wasm` (default): a `wasm-pack`-built browser module — the wasm-bindgen
///   `App` wrapper on, a standalone `[workspace]`, a `cdylib`.
/// - `Live`: an rlib to embed in a server (the sutegi-zumar bridge). Same
///   code, but wasm-bindgen is compiled *out* (the `wasm` feature is off by
///   default) and only `pub fn program()` is exposed. No new codegen — the
///   two targets differ by a feature default and the crate manifest.
...
pub fn generate(app: &App, deps: &str) -> Generated {
    generate_with(app, deps, Target::Wasm)
}

/// [`generate`] with an explicit [`Target`].
pub fn generate_with(app: &App, deps: &str, target: Target) -> Generated {
    let crate_name = app.name.to_lowercase();
    let g = Gen::new(app);
    Generated {
        cargo_toml: cargo_toml(&crate_name, deps, target),
        lib_rs: g.lib_rs(app),
        crate_name,
    }
}

fn cargo_toml(name: &str, deps: &str, target: Target) -> String {
    // Both targets share one lib.rs; only the manifest differs. `live` keeps
    // wasm-bindgen out of the build (feature off) and omits `[workspace]` so
    // the crate embeds cleanly as a path dependency inside a server's tree.
    let (crate_type, default_feat, workspace) = match target {
        Target::Wasm => ("\"cdylib\", \"rlib\"", "\"wasm\"", "\n[workspace]\n"),
        Target::Live => ("\"rlib\"", "", ""),
    };
    format!(
        r#"[package]
